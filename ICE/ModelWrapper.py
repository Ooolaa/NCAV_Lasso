...
class SparsityLayer(nn.Module):
    def __init__(self, in_channels, out_channels, height=1, width=1, alpha=1e-4):
        super(SparsityLayer, self).__init__()
        self.fc = nn.Linear(in_channels * height * width, out_channels * height * width)
        self.alpha = alpha
        self.height = height
        self.width = width
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, self.out_channels, self.height, self.width)
        return x

    def l1_penalty(self):
        return self.alpha * torch.sum(torch.abs(self.fc.weight))
...
class PytorchModelWrapper(ModelWrapper):   
    ...
    def get_feature(self, x, layer_name):
        ...
        sparsity_layer = SparsityLayer(in_channels=out.size(1), out_channels=out.size(1), height=out.size(2), width=out.size(3))
        if self.CUDA:
            sparsity_layer = sparsity_layer.cuda()
        out = sparsity_layer(out)
        return out
